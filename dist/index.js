...
var import_proxy_addr2 = __toESM(require("proxy-addr"));
var import_netparser3 = require("netparser");

// src/utils.ts
var import_proxy_addr = __toESM(require("proxy-addr"));
...
function compileProxyTrust(val) {
  ...
  return import_proxy_addr.default.compile(val);
}
function httpGET(url) {
  return new Promise((resolve, reject) => {
    import_node_https.default.get(url, { timeout: 5e3 }, (res) => {
      let data = "";
      res.on("data", (chunk) => {
        data += chunk;
      });
...
function logBlockedRequest(moduleName, req, referenceID, config5) {
  if (config5.disableLogging) return;
...
// src/modules/blockTorExitNodes.ts
var import_netparser = require("netparser");
var config;
var torExitNodes;
async function updateTorExitNodesList() {
  try {
    ...
    torExitNodes = new import_netparser.Matcher(arr);
  } catch (err) {
...
// src/modules/specialchars.regex.ts
...
// src/modules/directoryTraversal.ts
...
// src/modules/fakeCrawlers.ts
var import_promises = require("dns/promises");
var import_netparser2 = require("netparser");
var config2;
var uaRegex = new RegExp(
...
var fakeCrawlers_default = {
  ...
  check: async (req) => {
    ...
    try {
      const hostnames = await (0, import_promises.reverse)(req.ip);
      ...
      for (const hostname of matchedHostname) {
        const lookupRes = await (0, import_promises.lookup)(hostname);
        if (!lookupRes) {
          continue;
        }
...
async function updateIPWhitelist() {
  try {
    ...
    ipWhitelist = new import_netparser2.Matcher(json);
  } catch (err) {
...
function addIPToWhitelist(ip) {
  if (typeof ipWhitelist === "undefined") {
    ipWhitelist = new import_netparser2.Matcher();
  }
  ipWhitelist.add(ip);
}

// src/modules/httpParameterPollution.ts
...
// src/modules/prototypePollution.ts
...
// src/block.ts
async function block(req, res, moduleName, config5) {
  const date = /* @__PURE__ */ new Date();
  const referenceID = sha256(req.ip + date.getTime());
  if (typeof config5.preBlockHook === "function" && await config5.preBlockHook(req, moduleName, req.ip) === false) {
    return false;
  }
  if (!config5.dryMode) {
    res.writeHead(403, { "Content-Type": "text/html" });
    if (!config5.customBlockedPage) {
      res.write(
      ...
      res.write(
        config5.customBlockedPage.replace(/{\w+}/g, (matched) => {
  ...
  logBlockedRequest(moduleName, req, referenceID, config5);
  if (typeof config5.postBlockHook === "function") {
    await config5.postBlockHook(req, moduleName, req.ip);
  }
  if (config5.dryMode) {
    return false;
  }
...
function easyWaf(conf) {
  if (typeof conf === "object" && conf !== null && !Array.isArray(conf)) {
    ...
    if (typeof conf.ipBlacklist !== "undefined") {
      ...
      ipBlacklist = new import_netparser3.Matcher(conf.ipBlacklist);
    }
    if (typeof conf.ipWhitelist !== "undefined") {
      ...
      ipWhitelist2 = new import_netparser3.Matcher(conf.ipWhitelist);
    }
    conf.modules = { ...config4.modules, ...conf.modules };
    config4 = { ...config4, ...conf };
  }
  trustProxy = compileProxyTrust(typeof config4.trustProxy !== "undefined" ? config4.trustProxy : []);
  for (const [, module2] of Object.entries(modules_exports)) {
    if (typeof module2.init === "function") {
      module2.init?.(config4);
    }
  }
  return async function EasyWaf(rawReq, res, next) {
    const ip = (0, import_proxy_addr2.default)(rawReq, trustProxy);
    ...
    for (const [moduleName, module2] of Object.entries(modules_exports)) {
      ...
      const ok = await module2.check(req);
